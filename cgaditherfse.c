//TODO make this an 'e' option for cgaditherfs2.c
// it basically just makes the color range wider to prefer cyan/green a bit less
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
inline void _proc_ditherChroma(tddata_t* mydata)
{
    ...
    for(j = 0; j < mydata->img.w; ++j) {
        float Nu = MY(err, j);
        float old = MY(c, j);
        float new = round(CLAMP(4.f*(old + Nu), -4.f, 4.f));
        float err = 4.f*old - new;
        DISTRIBUTE_ERR(err, j);
        MY(c, j) = new;
    }
}
